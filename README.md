...
**This solution uses 0.30 MB of that budget** and decides each fragment in
...
| metric | value |
|---|---|
| combined model assets | **0.304 MB** (limit: 25 MB) |
| latency per fragment (laptop CPU) | **~2 ms** |
| false-reject rate, template-unseen test set | **0.21%** |
| forward precision @ recall 0.976 | 0.994 (threshold 0.75) |
| macro-F1, 6-class, template-unseen | 0.733 (rule baseline: 0.444) |
| scenario suite (whole engine, hand-written) | 18/22 turns |

Full analysis including the four scenario failures and both error-cost
directions: `docs/evaluation_report.md`.

## Submission statement (per assignment §10)

- **Implemented:** synthetic data pipeline, model comparison + training,
  gate engine (thresholds, uncertainty band, duplicate suppression,
  back-channel filter, session state), incremental simulator, FastAPI service,
  browser demo console, full evaluation with failure analysis.
...
- **Total combined model size:** 0.304 MB (see `models/model_size_report.txt`).
